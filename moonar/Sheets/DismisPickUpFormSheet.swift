...
struct DismissPickUpFormSheet: View {
			...
			ScrollView(showsIndicators: false) {
				...
				VStack(alignment: .leading, spacing: 0) {
					ForEach(pickUpList.indices, id: \.self) { index in
						DismissListItem(sheetPickedUpValue: $sheetPickedUpValues[index], color: $color, textColor: $textColor, text: pickUpList[index])
					}
				}
				.padding(10)
